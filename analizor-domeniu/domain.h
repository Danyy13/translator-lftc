...
struct Symbol {
    ...
    union { // specific data for each kind of symbol
        // the index in function.locals for local vars
		...
		// the index in function.params for parameters
        int paramIndex;
...
Symbol *newSymbol(const char *name,SymbolKind kind);
